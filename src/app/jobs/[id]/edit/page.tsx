...
export default function EditJobPage() {
    ...
    useEffect(() => {
        const loadJob = async () => {
            ...
            try {
                ...
                if (user?.id !== job.createdByUserId) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        if (step < 4) {
            ...
            setSubmitting(true);
            try {
                await api.updateJob(jobId, formData, firebaseToken);
                router.push(`/jobs/${jobId}`);
            } catch (error) {
                console.error('Error updating job:', error);
                alert(t('jobDetail.updateError'));
            } finally {
                setSubmitting(false);
            }
        }
        ...
        <div className="min-h-screen bg-card">
            ...
            <div className="border-b border-border">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-bold text-foreground">{t('jobDetail.editJob')}</h1>
                </div>
            </div>
                    ...
                    {step === 4 && (
                        <div className="space-y-6">
                            ...
                            <label className="flex items-start gap-3 p-4 border border-border rounded-lg cursor-pointer hover:bg-muted">
                                ...
                                <p className="text-sm text-foreground">{t('jobDetail.confirmUpdate')}</p>
                            </label>
                        </div>
                    )}
                        ...
                        <Button className="ml-auto" disabled={submitting}>
                            {submitting ? (
                                <>
                                    ...
                                    {t('jobDetail.updating')}
                                </>
                            ) : (
                                step === 4 ? t('jobDetail.saveChanges') : t('post.actions.continue')
                            )}
                        </Button>
                    </div>
                </form>
