    ...
    return (
      <div className="min-h-screen bg-muted">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <h1 className="text-3xl font-bold text-foreground mb-4">{t('profile.signInRequiredTitle')}</h1>
          <p className="text-muted-foreground mb-8">{t('profile.signInRequiredDesc')}</p>
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        {/* Header with Tabs */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-foreground mb-2 bg-gradient-to-r from-primary via-secondary to-primary bg-clip-text text-transparent">
            {t('profile.title')}
          </h1>
        ...
        {activeTab === 'account' && (
          <div>
            {/* Profile Card */}
            <div className="bg-card rounded-2xl shadow-lg overflow-hidden border-2 border-border">
              {/* Banner */}
                ...
                {!isEditing ? (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-lg text-foreground">{user.userType}</p>
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm font-medium text-muted-foreground">{t('profile.workerTypes')}</label>
                      ...
                      <div className="space-y-1">
                        <label className="text-sm font-medium text-muted-foreground">{t('profile.country')}</label>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-lg text-foreground">{viewState?.name || parsedViewAddress.state || '-'}</p>
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm font-medium text-muted-foreground">{t('profile.address')}</label>
                        <p className="text-lg text-foreground">{parsedViewAddress.address || '-'}</p>
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm font-medium text-muted-foreground">{t('profile.city')}</label>
                        <p className="text-lg text-foreground">{parsedViewAddress.city || '-'}</p>
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm font-medium text-muted-foreground">{t('profile.number')}</label>
                        <p className="text-lg text-foreground">{parsedViewAddress.number || '-'}</p>
                      </div>

                      <div className="space-y-1">
                        <label className="text-sm font-medium text-muted-foreground">{t('profile.postcode')}</label>
                        <p className="text-lg text-foreground">{parsedViewAddress.postcode || '-'}</p>
                      </div>
                    ...
                    <div className="pt-6 border-t border-border">
                      <h3 className="text-lg font-semibold text-foreground mb-4">{t('profile.quickActions')}</h3>
                    ...
                    <div className="pt-6 border-t border-border">
                      ...
                      <Button
                        ...
                        className="border-red-700 text-red-600 hover:bg-red-950/50"
                      >
                  ...
                  <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <button
                            ...
                            className="w-full px-3 py-2 border border-border rounded-lg flex items-center justify-between hover:border-primary"
                          >
                            ...
                            <div className="border border-border rounded-lg shadow-sm bg-background">
                              <div className="p-2 border-b border-border">
                                <input
                                  ...
                                  className="w-full px-3 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary bg-card text-foreground placeholder:text-muted-foreground"
                                />
                              </div>
                              <div className="max-h-48 overflow-y-auto divide-y">
                                {filteredCountries.slice(0, 50).map((country) => (
                                  <button
                                    ...
                                    className={`w-full px-3 py-2 flex items-center gap-2 text-left hover:bg-primary/10 ${formData.country === country.code2 ? 'bg-primary/10' : ''}`}
                                  >
                                    <span>{countryFlag(country.code2)}</span>
                                    <span className="flex-1">{country.name}</span>
                                    <span className="text-xs text-muted-foreground">{country.code2}</span>
                                  </button>
                                ))}
                                {filteredCountries.length === 0 && (
                                  <div className="px-3 py-2 text-sm text-muted-foreground">{t('profile.noResults')}</div>
                                )}
                              </div>
                            </div>
                          )}
                        ...
                        {selectedCountry ? (
                          <div className="space-y-2">
                            <button
                              ...
                              className="w-full px-3 py-2 border border-border rounded-lg flex items-center justify-between hover:border-primary"
                            >
                              <span className="text-foreground">{selectedCountry.states.find((s) => s.code === formData.state)?.name || t('profile.state')}</span>
                              <span className="text-muted-foreground">▾</span>
                            </button>

                            {showStateDropdown && (
                              <div className="border border-border rounded-lg shadow-sm bg-background">
                                <div className="p-2 border-b border-border text-sm text-foreground">
                                ...
                                <div className="max-h-48 overflow-y-auto divide-y">
                                  {(filteredStates || []).slice(0, 80).map((state) => (
                                    <button
                                      ...
                                      className={`w-full px-3 py-2 flex items-center justify-between text-left hover:bg-primary/10 ${formData.state === state.code ? 'bg-primary/10' : ''}`}
                                    >
                          ...
                          <p className="text-sm text-muted-foreground">{t('profile.selectCountryFirst')}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">{t('profile.number')}</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">{t('profile.postcode')}</label>
                        <input
                          type="text"
            ...
            {showDeleteConfirm && (
              <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                  <div className="fixed inset-0 bg-muted0 bg-opacity-75 transition-opacity" onClick={() => !isDeleting && setShowDeleteConfirm(false)} />
